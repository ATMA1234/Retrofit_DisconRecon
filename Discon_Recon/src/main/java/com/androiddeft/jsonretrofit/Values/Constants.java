package com.androiddeft.jsonretrofit.Values;

public class Constants {
    public static final int LOGIN_SUCCESS = 1;
    public static final int LOGIN_FAILURE = 2;
    public static final int DISCON_LIST_SUCCESS = 3;
    public static final int DISCON_LIST_FAILURE = 4;
    public static final int DISCON_SUCCESS = 5;
    public static final int DISCON_FAILURE = 6;
    public static final int CONNECTION_TIME_OUT = 7;
    public static final int DISCONNECTION_DIALOG = 8;
    public static final int RECON_LIST_SUCCESS = 9;
    public static final int RECON_LIST_FAILURE = 10;
    public static final int RECON_SUCCESS = 11;
    public static final int RECON_FAILURE = 12;
    public static final int RECONNECTION_DIALOG = 13;

}
